...
export default function LoginLawyer() {
  ...
  async function submit(e) {
    e.preventDefault();
    try {
      const res = await axios.post('https://bailbridge-6.onrender.com/api/auth/login', { email, password, role: 'lawyer' });
      if (res.data.token) {
        localStorage.setItem('token', res.data.token);
        localStorage.setItem('role', 'lawyer');
      }
      alert('Login successful!');
      nav('/lawyer-legal-aid');
    } catch (err) {
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="login-page">
      <div className="login-card">
        ...
        <div style={{ marginTop: 16 }}>
          Don't have an account? <Link to="/signup-lawyer">Sign up</Link>
        </div>
      </div>
    </motion.div>
  );
